...
# Define four datasets (data types)
...
# value_pool = [rng.randrange(N) for _ in range(5)]                          # 5 unique values to be repeated in repeated_data
# repeated_data = [random.choice(value_pool) for _ in range(5 * m_1)]        # repeated values
...
# Plot Hash1 results: one figure with 2 subplots (1x2)
fig1, plotses1 = plt.subplots(1, 2, figsize=(12, 8))
...
# Plot Hash2 results: one figure with 2 subplots (1x2)
fig2, plotses2 = plt.subplots(1, 2, figsize=(12, 8))
...
def plot_fpr(n, N, c, k_values, tests, plot):
    ...
    # Find min indices and values
    hash1_min_idx = hash1_fprs[1:].index(min(hash1_fprs[1:])) + 2                               # +3 to match x-axis (k=3 start)
    hash1_min_val = min(hash1_fprs[1:])

    hash2_min_idx = hash2_fprs[1:].index(min(hash2_fprs[1:])) + 2
    ...
    # Mark the minima
    plot.scatter(hash1_min_idx, hash1_min_val, color='blue', s=75, zorder=5, label='min hash1')
